...
import { withMiddleware } from '@signaltree/core/enhancers';
...
describe('SignalTree Guardrails Enhancer', () => {
  ...
  describe('Performance Budgets', () => {
    ...
    it('should respect alert threshold', () => {
      tree = signalTree({ count: 0 }).with(withGuardrails({
        budgets: {
          maxUpdateTime: 10,
  ...
  describe('Memory Leak Detection', () => {
    ...
    it('should warn on high signal retention', async () => {
      ...
      const memoryWarning = report.issues.find(
        i => i.type === 'memory' && i.message.includes('High signal retention')
      );
  ...
  describe('Custom Rules', () => {
    it('should evaluate custom rules', () => {
      ...
      const report = tree.__guardrails.getReport();
      const nestingViolation = report.issues.find(
        i => i.message.includes('Path too deep')
      );
    ...
    it('should prevent functions in state', () => {
      tree = signalTree({ data: {} }).with(withGuardrails({
      ...
      const report = tree.__guardrails.getReport();
      const functionViolation = report.issues.find(
        i => i.message.includes('Functions cannot be stored')
      );
    ...
    it('should check payload size', () => {
      ...
      const report = tree.__guardrails.getReport();
      const sizeViolation = report.issues.find(
        i => i.message.includes('Payload too large')
      );
    ...
    it('should handle rule errors gracefully', () => {
      ...
      tree = signalTree({ data: 0 }).with(withGuardrails({
        customRules: [badRule],
        reporting: { console: false },
      }));
  ...
  describe('Reporting', () => {
    ...
    it('should aggregate similar warnings', () => {
      ...
      const report = tree.__guardrails.getReport();
      const rootReadViolations = report.issues.filter(
        i => i.message.includes('Reading from root')
      );
  ...
  describe('Integration Tests', () => {
    it('should work with middleware composition', () => {
      const customMiddleware = {
        pre: vi.fn(update => update),
        post: vi.fn((update, result) => result),
      };
      
      tree = signalTree({ count: 0 })
        .with(withMiddleware(customMiddleware))
        .with(withGuardrails());
      
      tree.$.count.set(1);
      
      expect(customMiddleware.pre).toHaveBeenCalled();
      expect(customMiddleware.post).toHaveBeenCalled();
    });
    ...
    it('should handle complex state updates', () => {
      ...
      tree = signalTree(complexState).with(withGuardrails({
      ...
      // Bulk update
      tree.update({
        users: complexState.users.map(u => ({ ...u, settings: { ...u.settings, theme: 'dark' } })),
