...
## Accessing Guardrails Telemetry

Guardrails attaches a dev-only API to your tree that you can use to surface live metrics in tooling or UI components.

```typescript
import { signalTree, type SignalTree } from '@signaltree/core';
import { withGuardrails, type GuardrailsAPI, type GuardrailsReport } from '@signaltree/guardrails';

type GuardrailsEnabledTree<T extends Record<string, unknown>> = SignalTree<T> & {
  __guardrails?: GuardrailsAPI;
};

const tree = signalTree(initial).with(withGuardrails());
const api = (tree as GuardrailsEnabledTree<typeof initial>).__guardrails;

let latest: GuardrailsReport | null = null;
if (api) {
  latest = api.getReport();
}
```

See the updated **Guardrails Monitoring** page in the demo application (`/guardrails`) for a full example that visualises budgets, issues, and hot paths in real time.
...
| [guardrails-v1.1-enhanced.ts](./guardrails-v1.1-enhanced.ts)             | v1.1 enhanced implementation _(new)_                             |
| [guardrails.spec.ts](./guardrails.spec.ts)                               | Comprehensive test plan                                          |
| [benchmark-harness.ts](./benchmark-harness.ts)                           | Performance validation harness _(new)_                           |
    ...
    customRules: [
      {
        ...
        test: (ctx) => {
          if (ctx.metadata?.source === 'serialization' && ctx.path.includes('token')) {
            return false;
          }
          return true;
        },
