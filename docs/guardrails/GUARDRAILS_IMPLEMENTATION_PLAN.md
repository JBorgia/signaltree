...
```ts
// Assumes signalTree imported from '@signaltree/core'
...
```ts
// Assumes GuardrailsAPI imported from '@signaltree/guardrails'
// On feature teardown (dev-only)
const guardrailsApi = (dashboardTree as { __guardrails?: GuardrailsAPI }).__guardrails;
