...
| #   | Case                       | 14.0.0 | Why                                                                                        |
| --- | -------------------------- | ------ | ------------------------------------------------------------------------------------------ |
| 1   | Undo a row edit            | ❌     | `updateOne` records nothing                                                                |
...
| 5   | Undo my typing             | ❌     | No coalescing window; `shouldSkip` drops, cannot merge                                     |
...
| 9   | Server writes not undoable | 🟡     | `pauseRecording()` works, but no authorship concept, and `realtime`/`events` do not use it |

**Two of nine work. Four of the top four do not.** That is the honest position,
and it is a much sharper statement than the previous audit's "17 of 32", which
counted debugging affordances alongside product requirements and let the ranking
do no work.
...
1. **Record collection mutations.** Without it, cases 1, 2 and 4 are impossible and
   the fit page's undo claims are unsupported. This is the one that decides whether
   undo is real. It needs a design decision about granularity — per-mutation entry
   versus coalesced-per-turn — which is RFC material.
