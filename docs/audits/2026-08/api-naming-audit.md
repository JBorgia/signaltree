...
14.1.1 is a clean break, so an alias kept for compatibility is an alias kept for no
...
two operations. Added in 14.1.1.
...
all three public ones are being **deleted** in 14.1.1, so this resolves itself.
...
**DONE in 14.1.1** — follows Zod now: `parseEvent` throws, `safeParseEvent` returns a
...
**DONE in 14.1.1**: events' type is now `WebSocketConnectionState`, which also matches its
...
**Removed in 14.1.1** — including from every doc code block, which the `lint:skills`
