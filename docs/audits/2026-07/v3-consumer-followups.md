...
**Audience:** an implementing agent. Each item is self-contained: *why → exact files → concrete change → acceptance*.
...
|---|---|---|---|---|---|
| **G1b** | Make `.computed()` slice names **typed** on `tree.$` (drop the `as any`) | **Type fix** | **Highest** | RFC 0001 §4 (minimize surface): makes a *shipped* primitive usable, adds **no** surface | ✅ **DONE** 2026-07-28 |
...
| G2c | `docs/rfcs/0006-status-predicates-and-placement.md` header → **Accepted**. Note: §5 had *already* recorded the decision (2026-07-24, shipped 12.1.0) — this was pure **header** drift, not an open question, so the analysis was left untouched and the audit's finding E is linked as the after-the-fact demand evidence. |
...
either document (it makes an *already-shipped* primitive credibly usable, which is why v3 rejected it —
...
- A **computed slice** attaches a derived projection to the collection *at declaration*, so it lives
...
- **When to use a slice vs a derived layer:** a slice's `compute` receives only that collection's
  `E[]`. Anything that needs *other* state (another collection, an external id signal) must stay a
...
not credibly typed.** `.computed()` is documented as *"slice names aren't on the static `tree.$` type yet,
so read it via `(tree.$.users as any).name()`"* (`docs/skills/using-signaltree/SKILL.md:101`,
...
was *retracted* because a typed consumer met this feature, rejected it, and hand-rolled a typed equivalent.
...
~30 sites *alongside* `status()` — the migration didn't reach that subsystem (audit M2). A migration
...
**Why:** part of v3's `LoadingState` survived because `status()` didn't obviously cover *service-level*
(Bluetooth pairing) and *per-entity* (per-thread message) loading — and there's **no per-entity status
...
- **Per-entity** load state (e.g. "is *this row* loading") → **not** a core primitive. Model it as a
...
(audit M3 — low severity; `.all().find()` inside a `computed` is *correct*, the accessors just save a wrap).
...
- **`null as Nullable<T>` is the canonical answer — there is no `leaf()`/value marker to reach for.**
  ...
  (`packages/core/src/lib/signal-tree.ts:273`), which is *why* `{}` becomes a node and `null` stays a
...
codes)", with one row: *`Property 'set' does not exist on type 'NodeAccessor<…>'`* → you initialized an
...
**Action:** none — this section exists so the option is on record as *considered, rejected, why*, not
...
- **A — enhancer preset** (`standardEnhancers({ treeName, isProduction })`): **docs recipe by default, not
  ...
  is a **recipe/doc, not new surface**. The case for *core* would have to be made explicitly under RFC 0006
...
- **C — selection read-model** (`selectedEntities`/`isMultiEdit`/`hasSelections`/`selectionCount` over
  ...
  "two ways"); and RFC 0006 **§3** — `settled` was *deferred* for want of demand despite being plausibly
