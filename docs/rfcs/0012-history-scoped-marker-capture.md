...
## 14.1.0 — the option was RENAMED to `recordHistory`

This RFC proposed the flag as `history: false` and it shipped under that name in 14.0.0.
Renamed in 14.1.0 because it collided with `form({ history: history() })`, and the two are
**different questions**:
...
nothing visible. See the 14.1.0 CHANGELOG.
