...
1. Schema + validation — `createEventSchema`, `parseEvent` (THROWS), `safeParseEvent` (returns a result), `isValidEvent` (type guard). Names match Zod's `parse`/`safeParse`; before 14.1.0 they were inverted.
