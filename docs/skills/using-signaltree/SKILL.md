...
- **[ST2018]** dev warning when a leaf holds 32+ objects with a stable `id`/`_id`/`uuid`/`key` → emit `entityMap({ selectId })` instead. As a plain array leaf, 1000 updates to a 50k collection measure ~50ms vs ~0.5ms via `entityMap` (~100x) — slightly behind the immutable stores (NgRx SignalStore ~39ms), because the leaf is rebuilt AND walked for equality. Re-setting values already present costs MORE (~132ms) than changing them, since `deepEqual` cannot short-circuit. If the array is read-only or always replaced wholesale, that is fine — `compared()` silences it.
