...
- `entityMap<T, K>()` — O(1) CRUD: `addOne`, `upsertOne`, `removeOne`, `setAll`, `byId`, `all`, `clear`. Predicates: `.empty()` (v10.3 canonical) / `.isEmpty()` (deprecated alias). Reads: `.count()`, `.has(id)`, `.where(pred)`, `.find(pred)`. Config: `entityMap<T,K>({ selectId, sortComparer })` — v10.5+ `sortComparer` keeps `all()`/`ids()` sorted (`@ngrx/entity` parity). `byId(id).field()` is body-granular (fan-out 1). `.computed('name', all => …)` slices materialize at runtime; read via `(tree.$.users as any).name()` (slice names aren't on the static `tree.$` type yet).
