...
- **[ST2018]** dev warning when a leaf holds 32+ objects with a stable `id`/`_id`/`uuid`/`key` → emit `entityMap({ selectId })` instead. As a plain array leaf, 1000 updates to a 50k collection measure ~73ms vs 0.25ms via `entityMap` (~300x) — worse than the immutable stores (NgRx SignalStore ~39ms), because the leaf is rebuilt AND walked for equality. If the array is read-only or always replaced wholesale, that is fine — `compared()` silences it.
