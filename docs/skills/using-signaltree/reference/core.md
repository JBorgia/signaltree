...
Key options live on `EntityConfig<E, K>`: `selectId: (entity) => entity.someKey` for a custom id key (e.g., `selectId: (p) => p.sku`); `sortComparer: (a, b) => …` (v10.5+, `@ngrx/entity` parity) to keep `all()` / `ids()` in a stable sorted order (`asMap()` stays insertion-order (renamed from `asMap()` in v15 — `map` read as a projection beside `all()`)); plus optional `beforeAdd` / `beforeUpdate` / `beforeRemove` lifecycle hooks.
