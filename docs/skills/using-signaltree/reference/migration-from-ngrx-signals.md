...
- There is no reverse opt-in. `@signaltree/callable-syntax` and its `/augmentation` entry were **deleted in 14.0.0** precisely because that augmentation re-introduced this same conflict. Leaves take `.set()` / `.update()`.
