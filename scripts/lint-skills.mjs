...
const SIGNALTREE_PATH_MAP = {
  ...
  '@signaltree/core/edit-session': [
    'dist/packages/core/src/edit-session.d.ts',
  ],
  ...
  '@signaltree/ng-forms/audit': [
    'dist/packages/ng-forms/src/audit/index.d.ts',
  ],
  ...
  '@signaltree/events/angular': [
    'dist/packages/events/src/angular/index.d.ts',
  ],
  '@signaltree/events/testing': [
    'dist/packages/events/src/testing/index.d.ts',
  ],
...
function assembleSnippet(block, blockId, stubNames = []) {
  ...
  const stubBlock = stubNames.length
    ? stubNames.map((n) => `declare const ${n}: any;`).join('\n') + '\n'
    : '';
  if (wrap) {
    return (
      header +
...
async function main() {
  ...
  if (missing.length > 0) {
    for (const m of missing) {
      logWarn(
        `Import spec ${m.spec} could not be resolved to a built d.ts (tried: ${m.candidates.join(', ')}). Blocks importing it will fail.`
      );
    }
  }
  ...
  logInfo(
    `Found ${allBlocks.length} ts/tsx block(s) across ${new Set(allBlocks.map((b) => b.filePath)).size} file(s); linting ${linted.length}, skipping ${skipped.length} (wrong/bad/skip marker).`
  );
...
`;
  ...
  if (errorsByBlock.size === 0) {
    logOk(
      `Checked ${linted.length} block(s) across ${new Set(linted.map((b) => b.filePath)).size} file(s), all passed.`
    );
    process.exit(0);
  }
  ...
  for (const [block, diags] of errorsByBlock) {
    ...
    console.error(
      color.bold(
        color.red(
          `✗ ${rel}:${block.startLine}  (fence #${blockIdx}, ${diags.length} error${diags.length === 1 ? '' : 's'})`
        )
      )
    );
    for (const d of diags) {
      ...
      console.error(
        `  ${color.yellow(`TS${d.code}`)} ${color.gray(loc)}  ${d.message.split('\n')[0]}`
      );
      if (d.snippet) {
