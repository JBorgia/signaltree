...
function ensureLatest() {
  try {
    ...
    console.log('[bench-stats:vanilla] generating latest stats...');
    execSync('node scripts/performance/micro-benchmarks-variance-vanilla.mjs', {
      stdio: 'inherit',
    });
  }
}
...
if (
  ...
  console.error(
    '[bench-stats:vanilla] Latest stats file is empty or missing results. Expected performance/results/micro/vanilla/latest.json'
  );
  process.exit(1);
}
if (updateBaseline) {
  writeBaseline(latest);
  console.log('[bench-stats:vanilla] baseline updated.');
  process.exit(0);
}
let baseline;
try {
  ...
  writeBaseline(latest);
  console.log('[bench-stats:vanilla] created new baseline.');
  process.exit(0);
}
if (
  !baseline ||
  ...
  writeBaseline(latest);
  console.log(
    '[bench-stats:vanilla] baseline was empty; replaced with latest.'
  );
  process.exit(0);
}
...
console.log(
  `\nVanilla Benchmark Stats Regression (median>${slowdownPct}% || stddev>${stddevIncreasePct}%)`
);
console.table(
  rows.map((r) => ({
    metric: r.label,
...
if (failures) {
  console.error(
    `[bench-stats:vanilla] FAILURE: ${failures} metric(s) exceeded thresholds.`
  );
  process.exit(1);
}
console.log('[bench-stats:vanilla] Success: no median/stddev regressions.');
process.exit(0);
