...
const { signalTree, entityMap, withEntities } = require('../../dist/packages/core');
...
class EntityPerformanceAnalyzer {
  ...
  testAddOne() {
    ...
    const result = this.measureOperation('addOne', (i) => {
    ...
    console.log(`  Mean: ${result.mean.toFixed(4)}ms, P95: ${result.p95.toFixed(4)}ms\n`);
    return result;
  }
  ...
  testAddMany() {
    ...
    const result = this.measureOperation('addMany', (i) => {
      const tree = signalTree({
        entities: entityMap({ selectId: (e) => e.id }),
      }).with(withEntities());
  ...
  testUpdateOne() {
    ...
    const entities = Array.from({ length: 1000 }, (_, i) => this.createTestEntity(i));
    tree.$.entities.addMany(entities);

    const result = this.measureOperation('updateOne', (i) => {
      const id = i % 1000;
      tree.$.entities.updateOne(id, { status: 'updated', name: `Updated-${i}` });
    }, 1000);
  ...
  testUpdateWhere() {
    ...
    const result = this.measureOperation('updateWhere', (i) => {
      tree.$.entities.updateWhere(
        (e) => e.metadata.priority === (i % 5),
  ...
  testRemoveOne() {
    ...
    const result = this.measureOperation('removeOne', (i) => {
      const tree = signalTree({
        entities: entityMap({ selectId: (e) => e.id }),
      }).with(withEntities());
      
      // Add entities
      const entities = Array.from({ length: 100 }, (_, idx) => this.createTestEntity(idx));
      tree.$.entities.addMany(entities);
  ...
  testQueries() {
    ...
    // Test all()
    const allResult = this.measureOperation('all()', () => {
      tree.$.entities.all()();
    }, 1000);
    this.results.queryAll = allResult;
    console.log(`  all(): ${allResult.mean.toFixed(4)}ms, P95: ${allResult.p95.toFixed(4)}ms`);

    // Test byId()
    const byIdResult = this.measureOperation('byId()', (i) => {
      tree.$.entities.byId(i % 1000);
    }, 1000);
    this.results.queryById = byIdResult;
    console.log(`  byId(): ${byIdResult.mean.toFixed(4)}ms, P95: ${byIdResult.p95.toFixed(4)}ms`);

    // Test where()
    const whereResult = this.measureOperation('where()', (i) => {
      tree.$.entities.where((e) => e.metadata.priority === (i % 5))();
    }, 500);
    this.results.queryWhere = whereResult;
    console.log(`  where(): ${whereResult.mean.toFixed(4)}ms, P95: ${whereResult.p95.toFixed(4)}ms`);

    // Test find()
    const findResult = this.measureOperation('find()', (i) => {
      tree.$.entities.find((e) => e.id === (i % 1000))();
    }, 500);
    this.results.queryFind = findResult;
    console.log(`  find(): ${findResult.mean.toFixed(4)}ms, P95: ${findResult.p95.toFixed(4)}ms\n`);
  }
  ...
  testLargeDataset() {
    ...
    const entities = Array.from({ length: 10000 }, (_, i) => this.createTestEntity(i));
    tree.$.entities.addMany(entities);
  ...
  testReactivity() {
    ...
    // Add some entities
    const entities = Array.from({ length: 100 }, (_, i) => this.createTestEntity(i));
    tree.$.entities.addMany(entities);
    ...
    console.log(`  100 updates with signal reads: ${time.toFixed(2)}ms (${(time / 100).toFixed(4)}ms avg)\n`);
  }
  ...
  async runAll() {
    ...
    try {
      ...
      console.log(`  addOne:      ${this.results.addOne.mean.toFixed(4)}ms / ${this.results.addOne.p95.toFixed(4)}ms`);
      console.log(`  updateOne:   ${this.results.updateOne.mean.toFixed(4)}ms / ${this.results.updateOne.p95.toFixed(4)}ms`);
      console.log(`  removeOne:   ${this.results.removeOne.mean.toFixed(4)}ms / ${this.results.removeOne.p95.toFixed(4)}ms`);
      
      console.log('\nBatch Operations (mean/p95):');
      console.log(`  addMany:     ${this.results.addMany.mean.toFixed(4)}ms / ${this.results.addMany.p95.toFixed(4)}ms`);
      console.log(`  updateWhere: ${this.results.updateWhere.mean.toFixed(4)}ms / ${this.results.updateWhere.p95.toFixed(4)}ms`);
      
      console.log('\nQuery Operations (mean/p95):');
      console.log(`  all():       ${this.results.queryAll.mean.toFixed(4)}ms / ${this.results.queryAll.p95.toFixed(4)}ms`);
      console.log(`  byId():      ${this.results.queryById.mean.toFixed(4)}ms / ${this.results.queryById.p95.toFixed(4)}ms`);
      console.log(`  where():     ${this.results.queryWhere.mean.toFixed(4)}ms / ${this.results.queryWhere.p95.toFixed(4)}ms`);
      console.log(`  find():      ${this.results.queryFind.mean.toFixed(4)}ms / ${this.results.queryFind.p95.toFixed(4)}ms`);
      
      console.log('\nLarge Dataset (10k entities):');
      console.log(`  Add all:     ${this.results.largeDataset.addTime.toFixed(2)}ms`);
      console.log(`  Query all:   ${this.results.largeDataset.queryTime.toFixed(2)}ms`);
      console.log(`  Batch update: ${this.results.largeDataset.updateTime.toFixed(2)}ms`);
      console.log(`  Batch remove: ${this.results.largeDataset.removeTime.toFixed(2)}ms`);
