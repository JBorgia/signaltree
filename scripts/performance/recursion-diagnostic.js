...
class RecursionDiagnostic {
  measureDeepOperations(depth, iterations = 100) {
    ...
    return {
      depth,
      iterations,
      creation: { avg: avg(timings.creation), min: min(timings.creation), max: max(timings.creation) },
      access: { avg: avg(timings.access), min: min(timings.access), max: max(timings.access) },
      total: { avg: avg(timings.total), min: min(timings.total), max: max(timings.total) },
    };
  }
  ...
  runDiagnostic() {
    ...
    for (const depth of depths) {
      ...
      console.log(`  Creation: ${result.creation.avg.toFixed(4)}ms avg (${result.creation.min.toFixed(4)} - ${result.creation.max.toFixed(4)}ms)`);
      console.log(`  Access:   ${result.access.avg.toFixed(4)}ms avg (${result.access.min.toFixed(4)} - ${result.access.max.toFixed(4)}ms)`);
      console.log(`  Total:    ${result.total.avg.toFixed(4)}ms avg (${result.total.min.toFixed(4)} - ${result.total.max.toFixed(4)}ms)`);
      console.log('');
    }
    ...
    for (const depth of depths) {
      ...
      console.log(`Depth ${depth}: ${result.heapUsedMB.toFixed(2)}MB total, ${(result.avgPerStructure * 1024).toFixed(2)}KB per structure`);
    }
    ...
    for (const depth of depths) {
      ...
      console.log(`Depth ${depth}: ${result.avgPerOp.toFixed(4)}ms avg, heap: ${result.heapBefore.toFixed(2)}MB → ${result.heapAfter.toFixed(2)}MB (Δ${result.heapDelta.toFixed(2)}MB)`);
    }
    ...
    const extremeVsBasic = ((extreme.total.avg - basic.total.avg) / basic.total.avg) * 100;
    const unlimitedVsBasic = ((unlimited.total.avg - basic.total.avg) / basic.total.avg) * 100;
    
    console.log(`  Extreme (15) vs Basic (5):   +${extremeVsBasic.toFixed(1)}% slower`);
    console.log(`  Unlimited (20) vs Basic (5): +${unlimitedVsBasic.toFixed(1)}% slower`);
    console.log('');
    ...
    if (creationRatio > accessRatio) {
      console.log(`  - Creation scales worse than access (${creationRatio.toFixed(1)}x vs ${accessRatio.toFixed(1)}x)`);
    } else {
      console.log(`  - Access scales worse than creation (${accessRatio.toFixed(1)}x vs ${creationRatio.toFixed(1)}x)`);
    }
