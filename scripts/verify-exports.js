...
const PACKAGES = ['core', 'ng-forms', 'callable-syntax', 'enterprise', 'guardrails'];
...
for (const pkg of PACKAGES) {
  ...
  if (packageJson.main && !fs.existsSync(path.join(distDir, packageJson.main))) {
    console.error(`❌ ${pkg}: main file "${packageJson.main}" does not exist`);
    errors++;
  }
  
  if (packageJson.module && !fs.existsSync(path.join(distDir, packageJson.module))) {
    console.error(`❌ ${pkg}: module file "${packageJson.module}" does not exist`);
    errors++;
  }
  
  if (packageJson.types && !fs.existsSync(path.join(distDir, packageJson.types))) {
    console.error(`❌ ${pkg}: types file "${packageJson.types}" does not exist`);
    errors++;
  }
    ...
    const verifyExportPath = (exportPath, exportKey) => {
      if (typeof exportPath === 'string') {
        ...
        if (!fs.existsSync(fullPath)) {
          console.error(`❌ ${pkg}: export "${exportKey}" points to non-existent file: ${exportPath}`);
          errors++;
        }
        ...
        for (const [condition, conditionPath] of Object.entries(exportPath)) {
          if (typeof conditionPath === 'string') {
            ...
            if (!fs.existsSync(fullPath)) {
              console.error(`❌ ${pkg}: export "${exportKey}" condition "${condition}" points to non-existent file: ${conditionPath}`);
              errors++;
            }
          }
        }
