...
describe('a legacy 1.0.0 payload against a 14.0.0 tree', () => {
  ...
  // WITHDRAWN WITH STATUS-DEL — "a legacy key that happens to still be readable
  // is honoured". The whole assertion was about the status field's legacy
  // migration.
});

describe('a 14.0.0 payload round-trips completely (the control)', () => {
  // STATUS-DEL: the status specimen was trimmed here. `count` (plain) and `rows`
...
// WITHDRAWN WITH STATUS-DEL — the whole "ST2024 is a payload problem, not a
// registration one" suite. Its only case reported an unrecognised state on a
// status node; the subject is status-specific payload validation.
