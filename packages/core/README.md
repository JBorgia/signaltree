...
- `entityMap` reads: `all()`, `count()`, `ids()`, `asMap()` (the collection as a
  `ReadonlyMap`, keyed by id — renamed from `map()` in 14.1.0, which read as a
...
// `batchUpdate()` was REMOVED in 14.1.0 — it was a duplicate of the tree callable
