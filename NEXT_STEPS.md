# SignalTree Improvement Rollout Plan return true;
...
--- if (this.PROTOTYPE_KEYS.has(key)) {

      return false;

## Phase 0 – Baseline & Preparation (Week 0) }

- Benchmark current behaviour (lazy memory footprint, deep-update latency, CPU, bundle sizes). // Check against dangerous patterns

- Extract shared helpers into an internal `@signaltree/shared` module to pave the bundle-size pass. for (const pattern of this.DANGEROUS_PATTERNS) {

- Remove stub/no-op APIs and align naming conventions ahead of public announcements. if (pattern.test(key)) {

        return false;

**Deliverables:** benchmark snapshots, shared utilities package, cleaned public surface. }
...
## Phase 1 – Critical Stability (Week 1) if (key.includes('\0') || key.includes('\r') || key.includes('\n')) {

      return false;

1. **Lazy-tree Memory Manager** }

   - Implement `SignalMemoryManager` (WeakRef caches, FinalizationRegistry cleanup, stats API).

   - Pass manager into `createLazySignalTree` and expose `tree.dispose()` when lazy mode is active. return true;

2. **Prototype-Pollution Guardrails**

   - Introduce `SecurityValidator` with key vetting, optional string sanitisation, and `onSecurityEvent` callbacks.}

   - Apply validation in store creation + root mirroring loops (skip unsafe keys, log events).
...
4. **Docs & Tests**- Sanitizes string values to prevent XSS
   ...
   - Update README/migration notes for disposal/security/function rules. \*/

   - Add `memory.spec.ts`, `security.spec.ts`, `functions.spec.ts`; enable GC-enabled CI job (`node --expose-gc`). static sanitizeValue(value: unknown): unknown {

if (typeof value !== 'string') return value;

**Outcome:** no lazy-tree leaks, prototype pollution blocked, serialisable state guaranteed, docs/tests refreshed.

--- // Check for script injection attempts

    for (const pattern of this.INJECTION_PATTERNS) {

## Phase 2 – Performance Architecture (Week 2) if (pattern.test(value)) {

        console.warn(`[SignalTree Security] Potential injection detected in value: ${value.substring(0, 50)}...`);
...
1. **Optimised Update Engine** // Return sanitized version
   ...
   - Seed the index after creation and lazily via the memory manager for on-demand nodes. }

2. **Batching & Transactions** }

   - Enhance batching to support microtask/animation/idle/debounce strategies, rollbackable transactions, and `tree.transaction`, `tree.sync`, `tree.getBatchingStats` helpers.

   - Wrap every `WritableSignal.set/update` during enhancer setup so scheduling is transparent. return value;
...
- Creates a safe object that cannot be polluted

**Outcome:** opt-in high-performance updates, transparent batching, actionable diagnostics, validated perf gains. \*/

static createSafeObject<T extends object>(): T {

--- // Object.create(null) has no prototype chain

const obj = Object.create(null) as T;

## Phase 3 – Developer Experience & Features (Week 3)

1. **Computed Signals Enhancer** // Freeze the object's prototype to prevent modification

   - Ship `withComputed` backed by `ComputedSignalsEngine` (dependency tracking, memo/LRU cache, async support, circular detection) mounting results under `tree.computed` with invalidation helpers. if (Object.getPrototypeOf(obj)) {

2. **Edge-case Coverage & Bundle Optimisation** Object.freeze(Object.getPrototypeOf(obj));

   - Add tests for symbol keys, concurrent updates, recursion limits, large arrays. }

   - Use shared utilities to reduce bundle size (~20–30%) and document improvements.

3. **Documentation Refresh** return obj;

   - Update package docs + demo app to highlight batching, computed totals, diagnostics overlay, memory stats, serialisation rules.

4. **JSDoc Enhancements**}

   - Add rich JSDoc with `@example` blocks, parameter/return tags, internal/public markers for tooling support.

/\*\*

**Outcome:** richer DX without breaking APIs, extended test coverage, measurable bundle gains, refreshed documentation.

- Validates and sanitizes an entire object tree

--- \*/

static sanitizeObject<T extends object>(obj: T, options: SanitizeOptions = {}): SanitizedResult<T> {

## Phase 4 – Final Polish & Release (Week 4, optional) const errors: SecurityError[] = [];

const warnings: SecurityWarning[] = [];

- Finalise API consistency updates, publish migration guidance for any renamed helpers.

- Package releases:

  - Phase 1 ships as a backward-compatible minor. function traverse(current: any, path: string[] = [], depth = 0): any {

  - Phase 2 features stay flag-guarded until benchmarks validate default enablement. // Prevent infinite recursion

  - Phase 3+ roll into a coordinated major if public signatures change. if (depth > (options.maxDepth || 100)) {

- Monitor post-release diagnostics/perf metrics and plan follow-up patches as needed. errors.push({

          type: 'max_depth_exceeded',

**Outcome:** orderly release cadence, clear upgrade path, post-launch monitoring in place. path: path.join('.'),

          message: `Maximum depth ${options.maxDepth} exceeded`,
...
- [ ] Phase 0 complete (benchmarks + shared utilities + API cleanup) if (current === null || current === undefined) {

- [ ] Phase 1 complete (memory manager, security validator, function blocking, docs/tests) return current;

- [ ] Phase 2 complete (update engine, batching/transactions, diagnostics, benchmarks/tests) }

- [ ] Phase 3 complete (computed enhancer, edge tests, bundle optimisation, docs/JSDoc)

- [ ] Phase 4 complete (API polish, release packaging, monitoring) if (typeof current !== 'object') {

        return SecurityValidator.sanitizeValue(current);

Track progress against this list to ensure no deliverable is missed. }

      if (Array.isArray(current)) {
        return current.map((item, index) => traverse(item, [...path, String(index)], depth + 1));
      }

      const sanitized = SecurityValidator.createSafeObject();
      ...
      for (const [key, value] of Object.entries(current)) {
        ...
        if (!SecurityValidator.isSafeKey(key)) {
          errors.push({
            ...
            key,
            message: `Unsafe key "${key}" rejected`,
          });
        ...
        // Check for suspicious but allowed keys
        if (key.startsWith('_') || key.startsWith('$')) {
          warnings.push({
            type: 'suspicious_key',
            path: currentPath.join('.'),
            key,
            message: `Suspicious key "${key}" allowed but flagged`,
          });
        }

        (sanitized as any)[key] = traverse(value, currentPath, depth + 1);
      }
...
function createSecureSignalTree<T extends object>(obj: T, options: SecurityOptions = {}): SecureSignalTree<T> {
// Sanitize input
const sanitizeResult = SecurityValidator.sanitizeObject(obj, {
throwOnError: options.strict,
maxDepth: options.maxDepth || 100,
});

if (!sanitizeResult.isValid && options.strict) {
throw new SecurityError(`Input validation failed with ${sanitizeResult.errors.length} errors`);
}

// Log security events
if (sanitizeResult.errors.length > 0 || sanitizeResult.warnings.length > 0) {
...
// Create safe store
const store = SecurityValidator.createSafeObject<Record<string, WritableSignal<unknown>>>();
...
return signal(data);
    ...
    if (Array.isArray(data)) {
      return signal(data.map(buildTree));
    }

    const branch = SecurityValidator.createSafeObject();
...
const tree = buildTree(sanitizeResult.data);
...
describe('Security - Prototype Pollution Prevention', () => {
  it('should reject __proto__ key', () => {
    ...
    const result = SecurityValidator.sanitizeObject(malicious);
  ...
  it('should prevent constructor pollution', () => {
    ...
    expect(() => {
      createSecureSignalTree(malicious, { strict: true });
  ...
  it('should sanitize XSS attempts in values', () => {
    ...
    const tree = createSecureSignalTree(xssAttempt);
    expect(tree.name()).toBe('[SANITIZED]');
    expect(tree.safe()).toBe('normal text');
  });
  ...
  it('should handle deeply nested pollution attempts', () => {
    ...
    const result = SecurityValidator.sanitizeObject(nested);
